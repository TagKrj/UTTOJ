...
SITE_ID = 1
SITE_NAME = 'UTTOJ'
SITE_LONG_NAME = 'UTTOJ: Modern Online Judge'
SITE_ADMIN_EMAIL = ''


#Cấu hình xác thực hai lớp (2FA)
...
#Số lượng người dùng hàng đầu được hiển thị trên trang chủ là 5.
VNOJ_HOMEPAGE_TOP_USERS_COUNT = 5

#Định nghĩa các cấp bậc người dùng trong hệ thống, bao gồm người dùng thông thường, người tạo bài, nhân viên, quản trị viên, giáo viên, và người dùng bị cấm.
VNOJ_DISPLAY_RANKS = (
    ('user', _('Normal User')),
...
#Nếu vượt quá 50 test case, họ sẽ nhận cảnh báo (giới hạn mềm)
...
#Test case hiển thị tối đa 60 ký tự
...
#Tăng giới hạn số lượng trường dữ liệu tải lên lên 3000, vì một số bài tập có nhiều test case, mỗi test case có khoảng 5-6 trường
...
#ACE: Đường dẫn đến thư viện ACE Editor, một trình soạn thảo code
...
#Ngôn ngữ lập trình phải có ít nhất 10 bài nộp để xuất hiện trong thống kê
DMOJ_STATS_LANGUAGE_THRESHOLD = 10
#Định nghĩa màu sắc cho các kết quả nộp bài khác nhau
DMOJ_STATS_SUBMISSION_RESULT_COLORS = {
    'TLE': '#a3bcbd',# Time Limit Exceeded - màu xanh xám
    'AC': '#00a92a', # Accepted - màu xanh lá
    'WA': '#ed4420', # Wrong Answer - màu đỏ
    'CE': '#42586d',# Compilation Error - màu xanh đậm
    'ERR': '#ffa71c', # Error - màu cam
}
DMOJ_API_PAGE_SIZE = 1000

#Cửa sổ thời gian cho giới hạn đặt lại mật khẩu (3600 giây = 1 giờ)
DMOJ_PASSWORD_RESET_LIMIT_WINDOW = 3600
#Số lần tối đa có thể yêu cầu đặt lại mật khẩu trong cửa sổ thời gian (10 lần/giờ)
...
#Cấu hình địa lý và múi giờ
TIMEZONE_MAP = 'https://static.dmoj.ca/assets/earth.jpg'

TERMS_OF_SERVICE_URL = None
#Ngôn ngữ lập trình mặc định cho người dùng mới (C++17)
...
try:#cấu hình giao diện quản trị (admin interface) 
    import wpadmin
except ImportError:
    pass
else:
...
INSTALLED_APPS += (
    ...
    'django.contrib.admin', # Giao diện quản trị mặc định của Django
    'judge',#chứa tất cả mô hình dữ liệu và logic chính của hệ thống chấm bài, bao gồm quản lý bài tập, nộp bài, cuộc thi, người dùng và nhiều chức năng khác.
    'django.contrib.auth',# Hệ thống xác thực và phân quyền người dùng
    'django.contrib.contenttypes',
    'django.contrib.flatpages', # Quản lý các trang tĩnh
    'django.contrib.sessions', # Quản lý phiên người dùng và cookie
    'django.contrib.messages', # Hệ thống thông báo tạm thời
    'django.contrib.redirects',# Quản lý chuyển hướng URL
    'django.contrib.staticfiles', # Quản lý tệp tĩnh (CSS, JavaScript, hình ảnh...)
    'django.contrib.sites', # Hỗ trợ quản lý nhiều trang web từ một cơ sở dữ liệu
    'django.contrib.sitemaps',
    'registration',# Đăng ký tài khoản và kích hoạt email
    'mptt',# Cấu trúc dữ liệu cây cho phân cấp (như bình luận, danh mục)
    'reversion',
    'django_social_share', # Đăng nhập qua mạng xã hội (Google, GitHub, Facebook...)
    'social_django',
    'compressor',
    'django_ace', # Tích hợp trình soạn thảo mã ACE
    'sortedm2m',  # Mở rộng quan hệ many-to-many cho phép sắp xếp
    'statici18n',
    'impersonate',
    'django_jinja',
    'martor', # Trình soạn thảo Markdown nâng cao
    'adminsortable2',
...
AUTH_PASSWORD_VALIDATORS = [
    {
        'NAME': 'django.contrib.auth.password_validation.UserAttributeSimilarityValidator',#Đảm bảo mật khẩu không quá giống với thông tin cá nhân của người dùng (tên, email...)
    },
    {
        'NAME': 'django.contrib.auth.password_validation.MinimumLengthValidator',#Đảm bảo mật khẩu có đủ độ dài tối thiểu (mặc định là 8 ký tự)
    },
    {
        'NAME': 'judge.utils.pwned.PwnedPasswordsValidator',#So sánh mật khẩu với cơ sở dữ liệu Have I Been Pwned về các mật khẩu đã bị lộ trong các vụ rò rỉ dữ liệu
    },
    {
        'NAME': 'django.contrib.auth.password_validation.NumericPasswordValidator',#Ngăn chặn mật khẩu chỉ chứa toàn số
    },
]
...
PDF_STATEMENT_MAX_FILE_SIZE = 5242880 #5242880 byte = 5MB, hạn chế tệp PDF quá lớn
...
ENABLE_FTS = False #Tìm kiếm văn bản đơn giản, không sử dụng các thuật toán tìm kiếm toàn văn
...
try:
    ...
    pass
ALLOWED_HOSTS = ["*"]  # Chấp nhận tất cả host (chỉ dùng khi cần debug)
